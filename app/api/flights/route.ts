...
export async function GET(request: Request) {
    try {
        ...
        console.log(`Searching AirLabs: ${dep} to ${arr}`); // Look at your terminal for this log
        ...
        const response = await fetch(
            `https://airlabs.co/api/v9/schedules?dep_iata=${dep}&arr_iata=${arr}&api_key=${API_KEY}`
        );
