...
import {
    ...
    Users, Search, Loader2, Minus, Plus, Check
} from "lucide-react";
...
import { AIRPORTS } from "@/libs/data"; // Ensure this contains your airport list
...
export default function HeroSearch() {
    ...
    // --- SEARCH HANDLER ---
    const handleSearch = () => {
        // 1. Validation (UX Optimization)
        if (!origin || !destination) {
            // You can use a toast here later
    ...
    return (
        <div className="flex flex-col md:flex-row bg-white rounded-2xl md:rounded-full shadow-2xl p-2 border border-slate-100 w-full max-w-6xl mx-auto">

            {/* 1. SMART AUTOCOMPLETE: ORIGIN */}
            <LocationPicker
                label="Origin"
                value={origin}
                setValue={setOrigin}
            ...
            {/* 2. SMART AUTOCOMPLETE: DESTINATION */}
            <LocationPicker
                label="Destination"
                value={destination}
            ...
            <Button
                ...
                disabled={isLoading}
                className="bg-[#079d9a] hover:bg-[#068a87] text-white px-8 h-14 rounded-xl md:rounded-full font-bold shadow-lg shadow-[#079d9a]/20"
            >
                {isLoading ? <Loader2 className="animate-spin" /> : <>Find trip now <Search className="ml-2 h-4 w-4" /></>}
            </Button>
        </div>
    );
}
                ...
                <Popover open={open} onOpenChange={setOpen}>
                    ...
                    <PopoverContent className="w-[300px] p-0 bg-white border-none shadow-2xl rounded-xl overflow-hidden" align="start">
                        <Command className="bg-white">
                            <CommandInput placeholder={`Search ${label}...`} className="border-none focus:ring-0 text-slate-800" />
                            <CommandList>
                                ...
                                <CommandGroup heading="Suggestions" className="text-slate-400 font-bold px-2">
                                    {AIRPORTS.map((airport) => (
                                        <CommandItem
                                            ...
                                            className="flex items-center gap-3 py-3 px-3 cursor-pointer hover:bg-slate-50 rounded-lg text-slate-700"
                                        >
